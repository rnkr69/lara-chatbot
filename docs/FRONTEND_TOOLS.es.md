...
*[English](FRONTEND_TOOLS.md) · Español*

> Esta guía cubre el contrato `FrontendTool` (marker interface publicado en
...
>
> Lee primero [`docs/backend-tools.es.md`](backend-tools.es.md) — frontend tools heredan toda la
> mecánica de cascada de autorización + JSON Schema + `ToolResult` y sólo
> se diferencian en cómo el orquestador (`ChatService`) emite el
> evento SSE.
...
y se hereda. La diferencia con backend tools:

- **Backend tools** sólo soportan `Auto` end-to-end. El orquestador
  filtra y avisa por log.
- **Frontend tools** sí soportan los tres niveles — el `frontend_action`
  lleva el flag para que el widget decida si auto-ejecuta, pide
  confirmación al usuario o marca como manual.

Cómo aplicarlo: override `confirmation()` y devuelve el nivel deseado.
El widget interpreta:

| Nivel    | Comportamiento del widget |
|----------|--------------------------|
| `auto`   | Ejecuta inmediatamente al recibir el evento. |
| `confirm`| Muestra UI "¿Confirmas X?" antes de ejecutar. Persiste la acción pendiente. |
...
`window.Chatbot.registerNavigator(...)`; MPA cae a
...
sin guessing — ver [`integrations/backpack.es.md §5`](integrations/backpack.es.md).
Para forms custom (no-Backpack), publica el schema con la directiva
`@chatbotForm` — ver [`integrations/custom-forms.es.md`](integrations/custom-forms.es.md).
...
inline. Ver [`block-renderers.es.md`](block-renderers.es.md) para los renderers concretos.
...
El evento `Rnkr69\LaraChatbot\Events\ToolInvoked` se dispara también
...
| Aplicar tenant scope | Subclase + override `tenantScope(): bool` → true + bind un `TenantResolver`. Ojo: la cascada lo aplica antes del shim. |
