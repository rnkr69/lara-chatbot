...
> **Nota sobre versiones**: este documento contiene muchas referencias a
> milestones internos (`v2.0`, `v2.1.2 (#27)`, `E\d+`, etc.) del periodo
> pre-0.4 (2026-05-09 → 2026-05-16). Esas etiquetas son marcadores internos
> de ese periodo — no son releases públicas. Toda la funcionalidad descrita aquí está en
> la release `0.4.0` actual; las etiquetas internas se conservan porque los
> findings #N son punteros útiles al porqué de cada decisión.
...
> [`block-renderers.md`](block-renderers.md); la cascada de autorización en
> [`authorization.md`](authorization.md); el contrato de backend tools en
> [`backend-tools.md`](backend-tools.md); el `page_context` en
> [`page-context.md`](page-context.md). Esta guía referencia esos documentos
...
| `chart_renderer` | `'chartjs'` | `CHATBOT_DASHBOARD_CHART_RENDERER` | `'chartjs'` instala Chart.js como renderer por defecto del bloque `chart` en el bundle del dashboard. `'none'` deja el bloque sin renderer (el host registra el suyo vía `window.Chatbot.registerBlockRenderer('chart', fn)` antes del bundle). Ver §8. |
...
| `layout` | `null` | `CHATBOT_DASHBOARD_LAYOUT` | Si es string Y la vista existe, `chatbot::dashboard_layout` extiende ese layout (`@extends($layout) @section($section)`). Si null o no existe, se sirve `chatbot::dashboard` standalone. Mismo patrón que `chatbot.page.layout` (E17 / D16). **Sin un `layout` configurado el dashboard corre standalone — sin la navegación del host (ver §5.2).** |
| `section` | `'content'` | `CHATBOT_DASHBOARD_SECTION` | Sección donde inyectar el contenido al extender el layout del host. |
| `mount_widget` | `true` | `CHATBOT_DASHBOARD_MOUNT_WIDGET` | v2.1.1 (#26). En modo `layout`, monta el `<chatbot-widget>` flotante en la propia página del dashboard (vía `@push('after_scripts')`) para que el usuario pueda pinear **desde** el dashboard. Ponlo a `false` si el host inyecta el widget por su cuenta vía `extras_view`. Ver §5.2. |
| `back_url` | `null` | `CHATBOT_DASHBOARD_BACK_URL` | v2.1.1 (#26). URL del enlace "← volver a la app" que la vista **standalone** pinta arriba. `null` = sin enlace. En modo `layout` se ignora (la navegación la da el chrome del host). |
| `extras_view` | `null` | `CHATBOT_DASHBOARD_EXTRAS_VIEW` | v2.1.3 (#34). Nombre de una vista Blade del host (p. ej. `'admin._chatbot_widget'`) que `dashboard_layout.blade.php` `@include`a dentro de la sección, justo bajo el root del dashboard. Reemplaza al stack `chatbot_dashboard_extras` de v2.1.2 (#31), inalcanzable desde el `$layout` view por mecánica de Blade. La vista del host puede pintar markup directo o usar `@push('after_scripts')` (esta vez sí aterriza). Ver §5.2. |
...
concreta — un detalle de cliente, una vista de mercado), declara las
claves que necesita en `pageContextKeys()`:
...
El orquestador filtra el `page_context` activo a esas claves y las estampa
en `source.page_context_keys` del block. Al pinear, el endpoint captura el
subset filtrado en `source.page_context_snapshot`; al replay, el snapshot
se aplica al `ToolContext` antes de ejecutar. Sin esto, el replay devuelve
datos genéricos en lugar de los específicos al contexto.

Detalle completo en [`page-context.md` §7](page-context.md#7-page-context-en-pin-replay).
...
Pesos típicos en v2.0 al cierre de E9 (gzip):
- widget: ~26 KB / 80 cap (margen ~54 KB)
- dashboard: ~108 KB / 150 cap (margen ~42 KB)
...
> **v2.1.1 (#26) — el widget en modo `layout`.** En modo `layout`, si
...
>
> **v2.1.3 (#34) — inyectar el widget del host en modo `layout`.** El widget
...
> arregla #35: el bundle del widget detecta el shim que el bundle del
...
> funcionen.
>
> _Nota — el stack `@stack('chatbot_dashboard_extras')` que v2.1.2 (#31)
...
| `data-debug` | Controller (`app.debug`) | `1` / `0`. El controller lo sigue emitiendo desde `config('app.debug')` para forward-compat, pero el bundle ya no lo lee — v2.1.3 retiró el botón 👁 "View source" que dependía de él (limpieza del header de la card, ver release notes #33). |
...
### 5.6 Theming: Bootstrap host-native vs CSS propio (v2.1, #19)
...
cualquier driver es seguro. Ver `docs/deployment.md` §7.5.
...
6. Mapeo ToolResult → WidgetRefreshStatus (selección por descriptor, v2.1.2 #27):
...
> **v2.1.2 (#27) — selección de bloque en tools multi-bloque.** Un tool
...
en [`authorization.md`](authorization.md).
...
Detalle del shape esperado en [`block-renderers.md` §chart](block-renderers.md#chart).
...
`chatbot:dashboards:prune` (E10) hace housekeeping de filas inservibles
...
`docs/distribution.md §Pipeline CI #2` extiende esto al pipeline CI del
host con un snippet YAML genérico.
...
| Widget bundle | 18 KB gzip | ~26 KB gzip (con pin button + modal + setKpiLabels) | Re-publicar assets: `--tag=chatbot-assets --force`. |
...
`tests/Feature/Http/DashboardSecurityTest.php` (defensivos E10) o en los
tests originales E2–E8 referenciados.
...
| 4 | **Authorization 404-no-403** | Todos los endpoints aplican `Dashboard::forUser($user)` antes de `findOrFail`; widgets ajenos devuelven 404 incluso si el ID es válido. Cubierto por tests E4 (`it returns 404 when the dashboard belongs to another user`). | Confirme que `$user` no escapa al `Authorizer` del paquete (chat/dashboard comparten la cascada). |
| 5 | **`page_context_keys` filtering** | Al pinear, el server filtra `page_context` aplicando `source.page_context_keys` (estampado por el orquestador en E1 desde `pageContextKeys()` del tool). Claves NO declaradas son dropeadas; tras el filter aplica cap binario `chatbot.limits.page_context_kb` (default 16 KB, descarte completo + `Log::info`). Cubierto por `it filters page_context by source.page_context_keys before persisting the snapshot`. | Si una tool declara `pageContextKeys()`, marque ahí TODAS las claves que el handler espera leer; el orquestador no infiere — lo que no esté declarado se pierde al replay. |
| 6 | **`source.args` re-validation al replay** | El replay re-ejecuta `$tool->execute($args, $ctx)` cada vez; el JSON Schema del tool valida args en cada invocación. Si el cliente pinea con args válidos pero el tool tira al refresh (schema cambió, runtime error, edge case), el endpoint devuelve 200 con `last_refresh_status='error'` + `last_refresh_error` y conserva el snapshot anterior — **nunca 500**. Cubierto por `it refresh marks the widget as error … when the tool throws`. | Si una tool cambia su JSON Schema, los widgets pinneados antes del cambio degradan a `status=error` automáticamente — la UI ya sugiere "repinear desde el chat". |
| 7 | **Caps server-side (no opt-out)** | `max_dashboards_per_user` (default 20) y `max_widgets_per_dashboard` (default 50) se enforcen en los controllers — un cliente abusivo no puede crear infinitos rows. Cubiertos por `it rejects creating a dashboard above max_dashboards_per_user` y `it rejects pin when the dashboard already has max_widgets_per_dashboard widgets`. | Si su host atiende muchos usuarios, considere bajar los defaults vía config. |
| 8 | **Rate limit del replay** | `RateLimiter` por usuario sobre `refresh` + `refreshAll`; bulk SSE cuenta como 1 hit (no N por widget). Cubierto por `tests/Feature/Http/DashboardRefreshRateLimitTest.php`. **El CRUD NO entra al throttle** — limítelo desde su capa de proxy si lo necesita. | — |
...
|---|---|---|
| `add_to_dashboard` | `Auto` | Resuelve `source_tool`, lo ejecuta y pinea el block seleccionado al dashboard del user. PR-A. |
...
\* **Nota sobre confirmación**: el plan original (v2.2 doc) proponía
