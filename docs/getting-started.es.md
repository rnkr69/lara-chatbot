...
| Laravel | `^12.0` o `^13.0` | Ambas testeadas en CI. También `^11.0`, con un matiz (security-EOL — ver [Laravel 11](#laravel-11)). |
...
**Instalación.** El paquete está publicado en Packagist como
[`rnkr69/lara-chatbot`](https://packagist.org/packages/rnkr69/lara-chatbot), así
que basta un `composer require` (ver §2.1). Para distribución privada o
self-hosted (VCS, Satis, Packeton, Private Packagist) ver
[`distribution.es.md`](distribution.es.md).
...
Afecta a cualquier ruta `auth` del host, no sólo a las del chatbot.

---

## 2. Instalación rápida

### 2.1 Instala el paquete
...
El paquete **permite** Laravel 11 (`illuminate/* ^11.0|^12.0|^13.0`), pero Laravel 11
...
advisories en tu proyecto. La CI del paquete prueba Laravel 12 y 13 por este
motivo.

### 2.2 Ejecuta el wizard
...
[`WIDGET.es.md`](WIDGET.es.md) para la tabla completa.
...
> deliberadamente compactos; el detalle técnico vive en la documentación de cada
> componente.
...
Detalle completo en [`authorization.es.md`](authorization.es.md).
...
Detalle completo en [`FRONTEND_TOOLS.es.md`](FRONTEND_TOOLS.es.md).
...
| Necesito… | Lee |
|---|---|
| Entender la cascada permission → scope → tenant → ownership | [`authorization.es.md`](authorization.es.md) |
| Activar el Personal Dashboard (pinear bloques, modo `layout` vs standalone) | [`dashboard.es.md`](dashboard.es.md) |
| Construir tools de backend complejas (bulk, MCP, eventos) | [`backend-tools.es.md`](backend-tools.es.md) |
| Construir tools de frontend (formularios, navegación, descargas) | [`FRONTEND_TOOLS.es.md`](FRONTEND_TOOLS.es.md) |
| Renderizar bloques tipados (cards, tablas, gráficas) | [`block-renderers.es.md`](block-renderers.es.md) |
| Inyectar contexto de la página actual al LLM | [`page-context.es.md`](page-context.es.md) |
| Pedir confirmación antes de ejecutar acciones destructivas | [`confirmation-flow.es.md`](confirmation-flow.es.md) |
| Conectar servidores MCP externos | [`mcp.es.md`](mcp.es.md) |
| Customizar el widget (theming, slots, web component) | [`WIDGET.es.md`](WIDGET.es.md) |
| Integración con Backpack (admin) | [`integrations/backpack.es.md`](integrations/backpack.es.md) |
| Desplegar a producción (CDN, SSE detrás de proxy, rate limit) | [`deployment.es.md`](deployment.es.md) |
| Algo va mal en runtime | [`troubleshooting.es.md`](troubleshooting.es.md) |
| Distribuir versiones del paquete | [`distribution.es.md`](distribution.es.md) |
| Correr la suite de tests del paquete | [`testing.es.md`](testing.es.md) |
...
- [ ] Si usas la página de chat (`/chatbot`) o el Personal Dashboard
      ...
      salida. Ver [`dashboard.es.md`](dashboard.es.md).
